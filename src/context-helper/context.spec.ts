...
import { Context, ContextMerger, EmptyTemplatePathError, silentIfSameValue } from './context';
...
describe('ContextMerger', () => {
    let contextMerger: ContextMerger | undefined;
    afterEach(() => {
        contextMerger = undefined;
    });
        ...
        describe('Given two defined contexts', () => {
            ...
            describe('with no conflicted template context', () => {
                ...
                describe('When I call "mergeTemplateContext" on them', () => {
                    beforeEach(() => {
                        contextMerger = ContextMerger.of(instance(context1), instance(context2));
                        try {
            ...
            describe('with conflicted template context but with same value', () => {
                ...
                describe('and an overlapping merge strategy "silentIfSameValue"', () => {
                    beforeEach(() => {
                        contextMerger = ContextMerger.of(instance(context1), instance(context2))
                            .withTemplateMergeStrategy(silentIfSameValue);
                        try {
                            result = contextMerger.mergeTemplate();
                        } catch (error: unknown) {
                            exception = error as Error;
    ...
    describe('mergeTemplatePath', () => {
        ...
        describe('Given a set of valid context', () => {
            describe.each([
            ...
            ])('%s', (_description: string, templatePath: Array<string | string[]>, expectedAfterMerge: string[]) => {
                ...
                describe('When I call "mergeTemplatePath"', () => {
                    beforeEach(() => {
                        contextMerger = ContextMerger.of(...contexts.map(context => instance(context)));
                        try {
                            result = contextMerger.mergeTemplatePath();
                        } catch (error: unknown) {
                            exception = error as Error;
                        }
                    });
        ...
        describe.each([
        ...
        ])('Given %s', (_description: string, input: string | string[]) => {
            let context: Context;
            beforeEach(() => {
            ...
            describe('When I call "mergeTemplatePath"', () => {
                beforeEach(() => {
                    contextMerger = ContextMerger.of(instance(context));
                    try {
                        result = contextMerger.mergeTemplatePath();
                    } catch (error: unknown) {
                        exception = error as Error;
                ...
                test('Then the error is of type "EmptyTemplatePath"', () => {
                    expect(exception).toBeInstanceOf(EmptyTemplatePathError);
                });
            ...
            describe('And given 3 contexts', () => {
                ...
                describe('and all three context returned those 3 processors', () => {
                    ...
                    describe('When I call "mergeDestinationPathProcessor"', () => {
                        ...
                        beforeEach(() => {
                            contextMerger = ContextMerger.of(instance(context1), instance(context2), instance(context3));
                            resultingFunction = contextMerger.mergeDestinationPathProcessor();
                        });
